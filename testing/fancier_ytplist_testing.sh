...
# this is my test version. things may break (more often than in the """stable""" version.
# use at more of your own risk than the regular script
...
playlistify () { #functionizing the playlist generation

  now=$(date "+%Y-%m-%d-%M")
  cd "$dir"

  ls | grep -v \\.sh | grep -v \\.m3u >> "$now"_"$plname".m3u
}
...
convert2mp3 () { #this function is to be called if ffmpegPresence variable is not null
  echo -e "\nIt looks like you have ffmpeg installed."
  ...
  if [ "$convert" == "yes" ] || [ "$convert" == "y" ]
    then
      ...
      for i in *.webm ; do
        ...
        echo "Would you like a playlist of these mp3s while we're at it? Type \"yes\" or \"y\" to proceed with playlist generation."
        read playlistyn
        if [ "$playlistyn" == "yes" ] || [ "$playlistyn" == "y" ]
          then
            dir="mp3/"
            playlistify
            echo "Okay, mp3 playlist has been generated."
            echo "Bye!"
            exit 1
        else
          echo "Okay, no playlist generation this time. Bye!"
          exit 1
        fi
      done
    else
